...
def getMSE(X,Y,W):
    ...
    reg_t = (1/N)*(np.linalg.norm(W[:,1:])**2)
    
    # Calculating MSE
    
    MSE = (1/N)*(sum((Y-Fx)**2)) + reg_t
...
def fitData(X,Y,W,sigma2):
    ...
    for i in range(500):
        Fx = (X).dot(W.T)
        W[:,0] = W[:,0] + (0.003)*((2/N)*((Y - Fx).T).dot(X[:,0]))
        if d-1 > 0:
            W[:,1:] = W[:,1:] + ((0.003)*((2/N)*(((Y - Fx).T).dot(X[:,1:])+ (1)*(W[:,1:]))))    
...
def experiment(N,d,sigma2):
    ...
    W = np.zeros((1,d+1),dtype =np.float64)
    Ein =list()
    Eout =list()
    ...
    W_avg = np.mean(W,axis =0).reshape(1,X.shape[1])
...
N,d,sigma2 = [2, 5, 10, 20, 50, 100, 200],[*range(0,21)],[0.01**2,0.1**2,1**2]
data = np.zeros((1,6))
...
final_data_reg = pd.DataFrame(collected_data,
                          columns=['No of examples','Degree of polynomial','Variance','E_bias','Ein', 'Eout'])

# Saving collected data

final_data_reg.to_csv("Final Data_reg.csv")
